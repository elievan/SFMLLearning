...
void Map::Redraw(sf::Vector3i l_from, sf::Vector3i l_to) {
	...
	if (l_from.x >= mapSize.x || l_from.y >= mapSize.y) { return; }

	sf::Vector3i originalTo = l_to;
	if (l_to.x < 0 || l_to.x >= mapSize.x) { l_to.x = mapSize.x - 1; }
	if (l_to.y < 0 || l_to.y >= mapSize.y) { l_to.y = mapSize.y - 1; }
...
bool Map::ProcessLine(std::stringstream& l_stream) {
	...
	if(type == "TILE") {
		...
		m_tileSet.AddFile(Utils::GetWorkingDirectory() + "Build/media/Tilesheets/" + sheetName);
