...
void SoundManager::RemoveState(const StateType& l_state) {
	...
	for (auto &itr : StateSounds) { RecycleSound(itr.first, std::move(itr.second.second), itr.second.first.m_name); }
	m_audio.erase(l_state);
	auto music = m_music.find(l_state);
	if (music == m_music.end()) { return; }
	if (music->second.second) { --m_numSounds; }
	m_music.erase(l_state);
}

void SoundManager::Cleanup() {
	for (auto &state : m_audio) {
		for (auto &sound : state.second) { m_audioManager->ReleaseResource(sound.second.first.m_name); }
	}
	m_audio.clear();
	for (auto &recycled : m_recycled) { m_audioManager->ReleaseResource(recycled.first.second); }
	m_recycled.clear();
	m_music.clear();
...
{
	auto props = GetSoundProperties(l_sound);
	...
	SetUpSound(sound.get(), props, l_loop, l_relative);
...
bool SoundManager::LoadProperties(const std::string& l_name) {
	std::ifstream file;
	file.open(Utils::GetWorkingDirectory() + "Build/media/Sounds/" + l_name + ".sound");
	if (!file.is_open()) {
	...
	file.close();
	if (props.m_audioName.empty()) { return false; }
...
bool SoundManager::CreateSound(SoundID& l_id, const std::string& l_audioName, std::unique_ptr<sf::Sound>& l_sound) {
	if (!m_recycled.empty() && (m_numSounds >= Max_Sounds || m_recycled.size() >= Sound_Cache)) {
		...
		if (itr == m_recycled.end()) {
			...
			l_sound = std::move(element->second);
			l_sound->setBuffer(*m_audioManager->GetResource(l_audioName));
			m_recycled.erase(element);
		} else {
			l_id = itr->first.first;
			l_sound = std::move(itr->second);
	...
	if (m_numSounds < Max_Sounds) {
		if (m_audioManager->RequireResource(l_audioName)) {
			l_sound = std::make_unique<sf::Sound>();
			l_id = m_lastID;
			++m_lastID;
			++m_numSounds;
			l_sound->setBuffer(*m_audioManager->GetResource(l_audioName));
...
void SoundManager::RecycleSound(const SoundID& l_id, std::unique_ptr<sf::Sound> l_snd, const std::string& l_name)
{
